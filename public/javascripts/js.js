...
let pasteSelector; // area we we may paste element

// ACTION: get the whole data from mySQL in one shot 
// ACTION: add new line in cost menu inside subtotal price and distribute it over all cost menus in proportion to prices
// ACTION: hide columns
// ACTION: When hover over Description, then show product menu from DB
// ACTION: When type description, then search in db
// ACTION: When hover over description, ask somehow to add into
...
// ACTION: add template row & table into <template>
// ACTION: cookies replace with localStorage
// ACTION: nav bar do from the same elements, not from the duplicates
// ACTION: on td focus if no next add padding to place caret on bottom, remove it when get any value
// ACTION: split long first text to overall part --> boq --> pics, tables, vids. To let customers see tables immediately
// ACTION: check 'Embedly' + 'Font Awesome' + 'image_manager' plugin https://froala.com/wysiwyg-editor/docs/plugins/
// ACTION: make a commenting block for the customer where where customer and supplier can clarify something

function throttle (fn, delay = 200, isFinalEventConsidered = true) {
  let isThrottled = false;
  let finalArgs, finalThis; // args and this from final event

  function wrapper () {
    // ignore events during delay, but save args for the final event, even if it falls into delay slot
    if (isThrottled) {

      if (isFinalEventConsidered) {
        finalArgs = arguments;
        finalThis = this;
      }
      return;
    ...
    setTimeout(function() {
      ...
      if (isFinalEventConsidered && finalArgs) {
        wrapper.apply(finalThis, finalArgs); // final fn trigger // quite tricky
        finalArgs = finalThis = null;
      }
...
// toolbar not showing on one click and sometimes does not go away
(function fixHideShowToolBar () {

  let justSwitched = false

  document.addEventListener('mousedown', hideToolBar, { capture: true })
  
  function hideToolBar(e) {
    const frToolbar = document.querySelector('.fr-toolbar')
    const clickedOutsideFroalaDoc = !e.target.closest('.fr-element')
    if (clickedOutsideFroalaDoc) return 
    const clickedOnFrToolbar = e.target == frToolbar
    if (clickedOnFrToolbar) return 
    const isfrToolbarOn = frToolbar.style.display == 'block'
    if (isfrToolbarOn && !justSwitched) {
      frToolbar.style.display = 'none' 
      justSwitched = true
      setTimeout(() => { justSwitched = false }, 200)
...
// frArr.map( el => el.$box[0]) // list froala els

function initfrArr() {
  frArr = Array.from(document.querySelectorAll('#all-without-nav .froala')).map(el => returnfrEl(el))
}

function addTofrArr(el) {
  if (!el) return 
  const isInfrArr = frArr.length && frArr.some(frEl => frEl.$box[0] == el)
  if (isInfrArr) return
  frArr.push(returnfrEl(el))
}

function cleanfrArr() {
  const frElsOnPage = Array.from(document.querySelectorAll('#all-without-nav .froala'))
  frArr.forEach((frEl, i) => {
    const domElInFroalaObj = frEl.$box[0]
    if (!frElsOnPage.includes(domElInFroalaObj)) {
      frEl.destroy()
      frArr.splice(i, 1)
    }
  })
...
function returnfrEl(el) { 
  return FroalaEditor(el, {
    // initOnClick: true,
    toolbarInline: true,
    ...
    events: {
      ...
      'file.unlink': function (link) {
        ...
        if (removeFile) {
          // ACTION: remove file from DB
          // ACTION: check if any other offers or depend on the file
        }
      },

      // ACTION: check events for images and videos like for files
...
function cleanHtmlFromFroala(el) {
...
// $('.fr-toolbar').on('DOMSubtreeModified', function removeLegalNotice() {
//   const noticeEl = $(this).find('a[href*="froala"]')
//   if (noticeEl.length) {
//     noticeEl.remove()
//     console.log('removed')
//   }
    ...
    for (let i = undoArr.length - 1; i >= 0; i--) {
      if (undoArr[i].id == offer_id) {
        ...
        if (copyContainer.children().length)
          $(pasteSelector).addClass('cursor-copy');
    ...
    $.post(
      ...
      function (data) {
        if (data == 'no such offer') {
          ...
          // add copy pic to mouse pointer if we are in paste mode
          if (copyContainer.children().length) $(pasteSelector).addClass('cursor-copy');
...
$(document).on('keydown', '#all-without-nav', function (e) {
  ...
  if (undoCounter > 10) {
    ...
    const specialKyes = [ 37,38,39,40,35,36,33,34,27,112,113,114,115,116,117,118,119,120,121,122,123,16,17,91,18,173,174,175,144,8,46,];
...
$('#login').on('click', function (e) {
  if (!checkCookie('user_id')) {
    ...
    $(document).on('click', '#next', function () {
      ...
      if ( re.test($('#user_mail').val().toLowerCase()) == false || $('#user_mail').val() == '' || $('#user_pas').val() == '' ) {
        $('.logpass').addClass('shake')
        $('.logpass').one('animationend', () => {
          $('.logpass').removeClass('shake')
          showFooterTemp("mail patter in not correct");
        })
        return;
      }
...
$(document).on('click', '.menu-pic', function (e) {
  ...
  // menu position a bit to the right and down from click cursor
  ...
  menu
    .on('mouseenter', '.default', function () {
      ...
      let className;
      if ($(this).hasClass('add-btn')) className = 'shadow-blue-bottom';
      if ($(this).hasClass('delete-btn')) className = 'shadow-red-all';
      if ($(this).hasClass('copy-btn')) className = 'shadow-grey-all';
      if ($(this).hasClass('cut-btn')) className = 'shadow-grey-all-low-opacity';
      block.addClass(className) // highlight by applying the class
    })
  ...
  let copiedEl;
  menu.on( 'mouseenter', '.add-btn-txt, .add-btn-items, .add-btn-price, .add-btn-line', function (e) {
    e.stopPropagation();
    console.log( 'menu.on("mouseenter", ".add-btn-txt, .add-btn-items, .add-btn-price, .add-btn-line", function()' );
    if ($(this).hasClass('add-btn-txt')) copiedEl = $('#txt-template').clone();
    if ($(this).hasClass('add-btn-line')) copiedEl = $('#tr-template').clone();
    if ($(this).hasClass('add-btn-items')) copiedEl = $('#boq-template').clone();
    if ($(this).hasClass('add-btn-price')) copiedEl = $('#price-template').clone();
    
    copiedEl.removeAttr('id');
    copiedEl.removeClass('hidden');
    copiedEl.insertAfter(block);
    copiedEl.addClass('shadow-green-all');
  }).on( 'mouseleave', '.add-btn-txt, .add-btn-items, .add-btn-price, .add-btn-line', function (e) {
    console.log( 'menu.on("mouseleave", ".add-btn-txt, .add-btn-items, .add-btn-price, .add-btn-line", function(e)' );
    e.stopPropagation();
    copiedEl.remove();
  });
  ...
  menu.one( 'click', '.add-btn-txt, .add-btn-items, .add-btn-price, .add-btn-line', function (e) {
      e.stopPropagation();
      console.log( 'click on add btn - menu.one("click", ".add-btn-txt, .add-btn-items, .add-btn-price, .add-btn-line", function()' );
      cleanEl(copiedEl);
      // blink with green
      copiedEl.addClass('shadow-green-all');
      setTimeout(() => removeShadows(copiedEl), 500);

      if (copiedEl.hasClass('boq-block') || copiedEl.hasClass('row')) addToUndoArr();
      if (copiedEl.hasClass('txt-block')) addTofrArr(copiedEl[0].querySelector('.froala'));
  ...
  menu.one('click', '.delete-btn', function (e) {
    e.stopPropagation();
    console.log( 'click on delete btn - menu.one("click", ".delete-btn", function()' );
    ...
    if (!block.siblings().length) {
      block.addClass('shake')
      block.one('animationend', () => {
        block.removeClass('shake')
        removeShadows(block);
        underLayer.trigger('click');
  ...
  menu.one('click', '.copy-btn, .cut-btn', function (e) {
    ...
    let pasteEl, leaveAreaSelector, insertTo; 
    const allPasteAreaSelector = '#all-without-nav .block, .boq-block .block-table tbody tr';
    
    $(document).off('.copy_cut_namespace'); // we may copy element many times, but want to initiate events only ones, for that disable all possible existing click events
    putElInCopyContainer($(this));
    if (!copyContainer.children().length) return 
    setPasteAndLeaveAreaSelectors();
    showCopyContainer(e);
    showCopyCursor();
    underLayer.trigger('click'); // close

    $(document).on('mousemove.copy_cut_namespace', pasteSelector, throttle(whereToPaste, 100, false))
    $(document).on('mousemove.copy_cut_namespace', allPasteAreaSelector, throttle(showCopyContainer, 20, false))
    $(document).on('click.copy_cut_namespace', pasteSelector, paste)
    $(document).on('mouseleave.copy_cut_namespace', leaveAreaSelector, leaveArea)
    $(document).on('mouseenter.copy_cut_namespace', '.menu-pic', removeShadowsFromDivEls)

    function putElInCopyContainer(el) {
      removeShadows(block)
      const copiedEl = block.clone()
      const blockParentEl = block.parent()
      const isCutMode = el.hasClass('cut-btn')
      const isLastEl = block.siblings().length == 0

      if (isCutMode && isLastEl) {
      ...
      if (block.is('tr')) copiedEl.wrap('<table class="block-table"><div class="boq-block copied-row"></div></table>');

      copiedEl.show()
      copiedEl.addClass('slide-down')
      copyContainer.one('animationend', () => {
        copiedEl.removeClass('slide-down')
        showFooterTemp('copied');
      })
    }

    function showCopyCursor() {
      $('.cursor-copy').removeClass('cursor-copy');
      $(pasteSelector).addClass('cursor-copy');
    }

    function showCopyContainer(e) {
    ...
    function setPasteAndLeaveAreaSelectors() {
      ...
      if (pasteEl.hasClass('copied-row')) {
        pasteSelector = '.boq-block .block-table tbody tr';
        leaveAreaSelector = '.boq-block .block-table tbody';
      }
      else if (pasteEl.hasClass('block')) {
        pasteSelector = '#all-without-nav .block';
        leaveAreaSelector = '#all-without-nav';
      }
    }
    ...
    function removeShadowsFromDivEls(e) {
      ...
      copyContainer.hide();
      removeShadows($(pasteSelector));
    }

    function leaveArea(e) {
      e.stopPropagation();
      console.log('function leaveArea(e)');
      $(allPasteAreaSelector).removeClass("shadow-blue-top shadow-blue-bottom shadow-red-all");
      copyContainer.hide();
    }
    
    function whereToPaste(e) {
      e.stopPropagation();
      console.log('function whereToPaste(e)');
      ...
      // determine where to insert block and highlight area around with red or blue shadows
      ...
      // remove shadow to apply it again after calculation
      // removeShadows($(this).add($(this).prev()).add($(this).next()));
      $(allPasteAreaSelector).removeClass("shadow-blue-top shadow-blue-bottom shadow-red-all");

      if (distToUpperBorder / elHeight < 0.35 && $(this).is(':first-child')) {
        insertTo = 'VeryTop';
      ...
      if (distToLowerBorder / elHeight < 0.35) {
        insertTo = 'Under';
    ...
    function paste(e) {
      ...
      const dropIntoEl = $(this)

      // to prevent all other handlers
      $(document).off('.copy_cut_namespace');

      console.log( 'insert block - $(document).on("click", pasteSelector, function()');
      ...
      // insert
      firstElFromCopyContainer.addClass('slide-up')
      firstElFromCopyContainer.one('animationend', insertEl)
      
      function insertEl() {
        firstElFromCopyContainer.removeClass('slide-up')
        const isRow = firstElFromCopyContainer.hasClass('copied-row')
        const pastingEl = (isRow) ? firstElFromCopyContainer.find('tr').clone() : firstElFromCopyContainer.clone();

        cleanEl(pastingEl);
        pastingEl.addClass('slide-down');
        pastingEl.addClass('shadow-green-all');
        setTimeout(() => removeShadows(pastingEl), 700);

        if (insertTo == "Above") pastingEl.insertBefore(dropIntoEl);
        if (insertTo == "VeryTop") dropIntoEl.parent().prepend(pastingEl);
        if (insertTo == "Under") pastingEl.insertAfter(dropIntoEl);
        if (insertTo == "Middle") dropIntoEl.replaceWith(pastingEl.show());

        pastingEl.show();
        pastingEl.one('animationend', () => pastingEl.removeClass('slide-down'))

        firstElFromCopyContainer.remove();

        $(pasteSelector).removeClass('cursor-copy');
        boqPrice(pastingEl);
        blockPrices();
        addToUndoArr();
        addTofrArr(pastingEl[0].querySelector('.froala'))
        cleanfrArr()

        // if copyContainer is not empty, re-determine paste areas and re-apply listeners
        const isCopyContainerEmpty = !copyContainer.children().length
        if (isCopyContainerEmpty) {
          copyContainer.hide();
          return 
        }

        setPasteAndLeaveAreaSelectors(); // re-determine areas
        $(pasteSelector).addClass('cursor-copy'); // add copy pic to cursor
        // after area re-determination we need to re-launch all event handlers
        $(document).on('mousemove.copy_cut_namespace', pasteSelector, throttle(whereToPaste, 100, false));
        $(document).on('mousemove.copy_cut_namespace', allPasteAreaSelector, throttle(showCopyContainer, 50, false));
        $(document).on('mouseleave.copy_cut_namespace', leaveAreaSelector, leaveArea);
        $(document).on('click.copy_cut_namespace', pasteSelector, paste);
        $(document).on('mouseenter.copy_cut_namespace', '.menu-pic', removeShadowsFromDivEls);
      }
...
$(document).on('click', '.cost-menu-pic.tr', function (e) {
  ...
  menuEl.on('click', '.delete-btn', function (e) {
    ...
    if (!thisLine.siblings('.cost-row').length) {
      thisLine.addClass('shake')
      thisLine.one('animationend', () => {
        thisLine.removeClass('shake')
        showFooterTemp("can't delete last element");
      })
      return;
    }
